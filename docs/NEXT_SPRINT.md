...
| `src/agentic_rag/shared/config.py` | Central environment configuration with auth, database, object storage, OpenSearch, worker, and LLM budget/retry settings. | Add Redis, Kafka, reranker, object-store upload limits, observability, quota, and worker tuning settings. | High |
...
| `src/agentic_rag/llm/gateway.py` | LiteLLM-backed chat gateway with input/output request budget checks and transient provider retries used for grounded answer synthesis. | Add circuit breaker handling, model routing, prompt policy, provider-specific integration tests, and token/cost accounting hardening. | High |
...
| `tests/unit/llm/test_gateway.py` | LLM gateway unit tests, including LiteLLM call mapping, request budget rejection, and retry behavior. | Add provider-specific mocked cases, timeout/circuit-breaker tests, and no-secret logging tests. | High |
...
| Step | Work |
|---|---|
| 1 | Add LLM circuit breaker handling and provider health checks. |
