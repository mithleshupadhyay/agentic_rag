...
| 6 | Add query API that calls BM25 retrieval and context builder, but returns grounded retrieval output before LLM synthesis. |
| 7 | Add LLM gateway integration with provider/model env selection, budgets, retries, and citation-aware answer synthesis. |
| 8 | Add selective embedding worker with pgvector writes for only queries/documents that need semantic retrieval. |
| 9 | Add hybrid retrieval service: metadata, BM25, vector, merge, ACL filter, rerank, context build. |
| 10 | Add agent runtime skeleton with max steps, max_tool_calls, timeout, checkpointing, and loop protection. |
